...
| `npm run smoke:d0` | **Operational control plane, D0:** builds the agent image, runs it as a **researcher** container (role from `QMS_MANIFEST`), and proves it boots, answers `/health`, and self-registers with Discovery advertising `research:qms`. Needs Colima/Docker + Discovery. |
...
| `npm run integration:provisioning` | **Compute plane, D2a:** provisions a researcher **through the Provisioning API** (`POST /api/v1/instances`) backed by the Docker provider — it registers in Discovery, reports healthy, and `DELETE` destroys it; unauth is rejected. Swapping Docker for a cloud VM is one provider behind this API. Needs Colima/Docker + Discovery + ID Server + the agent image. |
| `npm run integration:api-launcher` | **Compute plane, D2b:** the Supervisor's `Launcher` as a client to the Provisioning API — `launch(manifest)` spawns a researcher (live in Discovery, healthy) and `stop(guid)` destroys it, with **no Docker knowledge in the Supervisor**. Needs Colima/Docker + Discovery + ID Server + the agent image. |
